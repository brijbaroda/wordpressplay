	...
	<!--	<h2 class="entry-title"><?php //the_title(); ?></h2> -->
		...
		<?php
			...
			if ( ! BizSphere_categorized_blog() ) {
				...
				if ( '' != $tag_list ) {
					$meta_text = ''.__( 'Tags : %2$s', 'BizSphere' ).''.__( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>', 'BizSphere' ).'';
				} else {
					$meta_text = ''.__( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>', 'BizSphere' ).'';
				}

			} else {
				// But this blog has loads of categories so we should probably display them here
				if ( '' != $tag_list ) {
					$meta_text = ''.__( 'Categories : %1$s', 'BizSphere' ).''.__( 'Tags : %2$s', 'BizSphere' ).''.__( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>', 'BizSphere' ).'';
				} else {
					$meta_text = ''.__( 'Categories : %1$s', 'BizSphere' ).''.__( 'Bookmark the <a href="%3$s" title="Permalink to %4$s" rel="bookmark">permalink</a>', 'BizSphere' ).'';
				}

			} // end check for categories on this blog
                      // printf('<div class="entry-meta-bottom-item">');
                        the_postPDFLink($post);
                       // printf('</div>');
			printf(
				$meta_text,
				$category_list,
